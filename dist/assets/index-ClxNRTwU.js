(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))t(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const s of o.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&t(s)}).observe(document,{childList:!0,subtree:!0});function i(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function t(a){if(a.ep)return;a.ep=!0;const o=i(a);fetch(a.href,o)}})();const r={lang:"cn",page:"mannayan"},l={};let c=null;function w(e,n){l[e]=n}function y(e,n=!0){var t;l[e]||(e="mannayan"),(t=c==null?void 0:c.unmount)==null||t.call(c);const i=document.getElementById("app");l[e].mount(i),c=l[e],r.page=e,Object.keys(l).forEach(a=>{var o;(o=document.getElementById("nav-"+a))==null||o.classList.toggle("active",a===e)}),n&&history.pushState({page:e},"","/"+e)}function M(){const e=location.pathname.replace(/^\//,"").split("/")[0],n=l[e]?e:"mannayan";y(n,!1),l[e]||history.replaceState({page:n},"","/"+n)}window.addEventListener("popstate",e=>{var n;y(((n=e.state)==null?void 0:n.page)||"mannayan",!1)});const g={en:{eyebrow:"Premium Nutritional Supplements",heroTitle:"Nature's <em>Intelligence</em>,<br>Scientifically Delivered",heroSub:"Food-state formulas crafted to work with your body's natural intelligence — pure, potent, and precisely made.",sectionTitle:"Our Collection",readMore:"Learn more",tabOverview:"Overview",tabDetails:"Details",tabIngredients:"Ingredients",footerRepTitle:"Representative",footerQrLabel:"WeChat",footerBottom:"© 2025 L&H Love and Healing GmbH · Authorised distributor of"},cn:{eyebrow:"高品质营养补充剂",heroTitle:"顺应自然<em>智慧</em>，<br>科学精准呈现",heroSub:"食物态配方，顺应身体自然吸收方式，纯净、高效、精准制造。",sectionTitle:"产品系列",readMore:"了解更多",tabOverview:"产品概述",tabDetails:"详细介绍",tabIngredients:"成分解析",footerRepTitle:"联系人",footerQrLabel:"微信",footerBottom:"© 2025 L&H Love and Healing GmbH · 官方授权经销商"}};function T(e){r.lang=e,document.getElementById("btn-en").classList.toggle("active",e==="en"),document.getElementById("btn-cn").classList.toggle("active",e==="cn"),C(),document.dispatchEvent(new CustomEvent("langchange",{detail:e}))}function C(){const e=g[r.lang],n=document.getElementById("footer-bottom");n&&(n.innerHTML=`${e.footerBottom} <a href="https://mannayan.com" target="_blank">mannayan.com</a>`);const i=document.getElementById("footer-rep-title");i&&(i.textContent=e.footerRepTitle);const t=document.getElementById("footer-qr-label");t&&(t.textContent=e.footerQrLabel)}function S(){document.querySelectorAll("[data-nav]").forEach(e=>{e.addEventListener("click",n=>{n.preventDefault(),y(e.dataset.nav)})}),document.querySelectorAll("[data-lang]").forEach(e=>{e.addEventListener("click",()=>T(e.dataset.lang))})}const A=[{name:"Mannayan 7&1+ (SPURENELEMENTE + JOD)",url:"https://mannayan.com/Mannayan-7-1-SPURENELEMENTE-JOD/011262",price:["27,90 €"],product_number:"011262",quantity:["90 Tabletten"],image_url:"https://mannayan.com/media/a5/d5/d9/1766136426/7%2B1%2B_Spuenelemente_2025.jpg?ts=1766136426",image_local:"images/7%2B1%2B_Spuenelemente_2025.jpg",content:{cn:{label:`Mannayan 7&1+ 微量元素+碘
为日常活力提供基础支持
• 支持能量代谢与免疫健康
• 促进认知功能与甲状腺平衡
• 作为多种酶的辅助因子
...
**Lactase**
Lactase is the enzyme specifically responsible for breaking down lactose (the sugar in milk). For individuals whose endogenous lactase activity has decreased due to aging or constitution, supplementing with lactase before consuming dairy products can prevent or alleviate discomfort like bloating and diarrhea associated with lactose intolerance. In the formula, it provides targeted support for dairy digestion, making the enjoyment of foods like cheese and yogurt easier.`}}}],m=A;let u=null,p=[];function d(e,n,i){e.addEventListener(n,i),p.push({el:e,event:n,handler:i})}function f(e){const n=e.split(`
`),i=n[0],t=n[1]||"",a=n.slice(2).filter(s=>s.startsWith("•")).map(s=>s.replace("•","").trim()),o=n.filter(s=>!s.startsWith("•")&&n.indexOf(s)>1);return{title:i,sub:t,bullets:a,footer:o}}function _(e){return e?e.split(/\n\n(?=\*\*)/).map(n=>{const i=n.match(/^\*\*(.+?)\*\*/),t=i?i[1]:"",a=n.replace(/^\*\*(.+?)\*\*\n?/,"").trim();return{name:t,desc:a}}).filter(n=>n.name):[]}function b(e){return e?e.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/\n\n/g,"<br><br>"):""}function v(){const e=document.getElementById("products-grid");if(!e)return;const n=g[r.lang];document.getElementById("hero-eyebrow").textContent=n.eyebrow,document.getElementById("hero-title").innerHTML=n.heroTitle,document.getElementById("hero-sub").textContent=n.heroSub,document.getElementById("section-title").textContent=n.sectionTitle,document.getElementById("section-count").textContent=`${m.length} ${r.lang==="cn"?"款产品":"products"}`,e.innerHTML="",m.forEach((i,t)=>{const a=i.content[r.lang],o=f(a.label),s=document.createElement("div");s.className="product-card",s.style.animationDelay=t*.08+"s",s.innerHTML=`
      ...
      <div class="card-body">
        ...
        <ul class="card-bullets">
          ${o.bullets.slice(0,4).map(x=>`<li>${x}</li>`).join("")}
        </ul>
      ...
      </div>`,s.addEventListener("click",()=>B(t)),e.appendChild(s)})}function B(e){u=e,I("overview"),k(),document.getElementById("modal-overlay").classList.add("open"),document.body.style.overflow="hidden"}function h(){document.getElementById("modal-overlay").classList.remove("open"),document.body.style.overflow=""}function k(){const e=m[u],n=e.content[r.lang],i=f(n.label),t=g[r.lang];document.getElementById("modal-img").src=e.image_url,document.getElementById("modal-img").alt=i.title,document.getElementById("modal-tag").textContent=`#${e.product_number}`,document.getElementById("modal-title").textContent=i.title,document.getElementById("modal-sub").textContent=i.sub||i.footer[0]||e.quantity[0],document.getElementById("modal-bullets").innerHTML=f(n.label).bullets.map(o=>`<li>${o}</li>`).join(""),document.getElementById("tab-overview").textContent=t.tabOverview,document.getElementById("tab-details").textContent=t.tabDetails,document.getElementById("tab-ingredients").textContent=t.tabIngredients,document.getElementById("modal-short").innerHTML=b(n.short_description),document.getElementById("modal-long").innerHTML=b(n.long_description);const a=_(n.ingredient_reference);document.getElementById("modal-ingredients").innerHTML=a.map(o=>`
    <div class="ingredient-card">
      <div class="ingredient-name">${o.name}</div>
      <div class="ingredient-desc">${o.desc}</div>
    </div>`).join("")}function I(e){["overview","details","ingredients"].forEach(n=>{document.getElementById("tab-"+n).classList.toggle("active",n===e),document.getElementById("panel-"+n).classList.toggle("active",n===e)})}function z(){return`
<section class="hero">
...
</div>`}function P(e){e.innerHTML=z(),["overview","details","ingredients"].forEach(t=>{d(document.getElementById("tab-"+t),"click",()=>I(t))}),d(document.getElementById("modal-close-btn"),"click",h),d(document.getElementById("modal-overlay"),"click",t=>{t.target===document.getElementById("modal-overlay")&&h()}),d(document,"keydown",t=>{t.key==="Escape"&&h()}),d(document,"langchange",()=>{v(),u!==null&&document.getElementById("modal-overlay").classList.contains("open")&&k()}),v()}function D(){p.forEach(({el:e,event:n,handler:i})=>e.removeEventListener(n,i)),p=[],u=null,document.body.style.overflow=""}const F=Object.freeze(Object.defineProperty({__proto__:null,mount:P,unmount:D},Symbol.toStringTag,{value:"Module"}));function E(e){e.innerHTML=`
<section class="hero hero--static hero--full">
  ...
  <div class="hero-rule"></div>
</section>`}function N(){}const O=Object.freeze(Object.defineProperty({__proto__:null,mount:E,unmount:N},Symbol.toStringTag,{value:"Module"}));w("mannayan",F);w("timewaver",O);S();T("cn");M();
